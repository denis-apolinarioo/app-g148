...
export async function verificarConquistas(uid, streakAtual, contexto, dracmaSaldoAtual = null) {
  ...
  // PERFORMANCE: cada conquista candidata é checada de forma independente
  // das outras (nenhuma lê ou escreve em algo que outra também mexe), então
  // rodam todas em paralelo em vez de uma esperando a outra terminar (era um
  // `for` com `await` sequencial — uma leitura no Firestore por conquista,
  // uma de cada vez). Com o catálogo podendo chegar a dezenas de conquistas,
  // isso é o que fazia essa checagem — chamada depois de publicar um post ou
  // enviar uma missão — demorar vários segundos a mais. O resultado final é
  // idêntico: mesma lista de conquistas novas, mesmas notificações.
  const resultados = await Promise.all(
    candidatas.map(async (conquista) => {
      const jaTem = await jaDesbloqueada(uid, conquista.id);
      ...
      if (multiContador) {
        if (!contadorRelevanteParaMulti(conquista.contadores, contexto)) return null;
        return avaliarConquistaMultiContador(uid, conquista, streakAtual, dracmaSaldoAtual);
      }

      if (!contadorRelevantePara(conquista.contadorTipo, contexto)) return null;

      const valorAtual = await calcularContador(conquista.contadorTipo, uid, streakAtual, dracmaSaldoAtual);
      if (valorAtual >= conquista.meta && (await desbloquear(uid, conquista))) {
        await notificarConquista(uid, conquista);
        return conquista;
      }
      return null;
