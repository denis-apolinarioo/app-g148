...
/**
 * ATUALIZAÇÃO CONQUISTAS — credita a recompensa (Pontos de Comunhão e/ou
 * Dracma) configurada na conquista, no momento em que ela é desbloqueada.
 * Duas transações SEPARADAS (nunca uma só mexendo em `pontos` E `dracmas`
 * ao mesmo tempo) porque a regra do Firestore em users/{uid} só aceita
 * update que muda SÓ `pontos` OU SÓ `dracmas` — igual já funciona em
 * lib/points.js (awardPoints) e lib/dracma.js (creditarDracma), que também
 * nunca combinam os dois num update só. Ver firestore.rules.
 *
 * Ficam aqui (em vez de chamar awardPoints/creditarDracma direto) pra
 * evitar import circular: lib/dracma.js e lib/notificacoes.js já importam
 * verificarConquistas deste arquivo. `uid` no log (não `origem`/`destino`)
 * porque a regra de pointsLog/dracmaLog já aceita `uid == auth.uid` (quando
 * a própria pessoa desbloqueia) OU `isAdmin()` (quando é concedida
 * manualmente por outra pessoa — ver concederConquistaManualmente).
 *
 * Nunca trava o desbloqueio por causa de erro ao creditar — mesma postura
 * de "nunca travar a tela" do resto do arquivo.
 */
async function creditarPontosConquista(uid, valor, conquista) {
  if (!valor) return;
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.set(logRef, {
        ...
        tipo: 'conquista',
        valor,
        descricao: `Conquista: ${conquista.nome || conquista.id}`,
        referenciaId: conquista.id,
        createdAt: serverTimestamp(),
      });
    });
  } catch (err) {
    console.error('Erro ao creditar pontos de conquista:', err);
  }
}

async function creditarDracmaConquista(uid, valor, conquista) {
  if (!valor) return;
  try {
    const userRef = doc(db, 'users', uid);
    ...
    console.error('Erro ao creditar Dracma de conquista:', err);
  }
}

/**
 * Recebe a conquista INTEIRA (não só o id) porque, além de gravar o
 * desbloqueio, agora também credita a recompensa configurada nela — ver
 * creditarPontosConquista/creditarDracmaConquista acima.
 */
async function desbloquear(uid, conquista) {
  ...
  // ATUALIZAÇÃO CONQUISTAS — recompensa opcional (0 por padrão em conquista
  // antiga, não muda nada pra quem já existia). Igual pra automática e
  // manual: concederConquistaManualmente também passa por este desbloquear().
  await creditarPontosConquista(uid, conquista.pontosRecompensa || 0, conquista);
  await creditarDracmaConquista(uid, conquista.dracmaRecompensa || 0, conquista);
  return true;
}
...
async function calcularContador(contadorTipo, uid, streakAtual, dracmaSaldoAtual = null, desde = null) {
  ...
  if (contadorTipo === 'dracma_ganho_total') {
    ...
    // Admin); a partir desta atualização, também 'conquista' (recompensa de
    // conquista desbloqueada — ver creditarDracmaConquista acima).
    // 'transferencia' fica de fora de propósito (não é "ganho", é recebido
    // de outra pessoa — ver dracma_recebido_total).
    return somarDracmaLog(uid, ['categoria', 'ajuste_admin', 'conquista'], desde);
  }
