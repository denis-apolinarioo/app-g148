...
async function calcularContador(contadorTipo, uid, streakAtual, dracmaSaldoAtual = null) {
  if (contadorTipo === 'streak') {
    if (streakAtual != null) return streakAtual;
    // Sem o streak em mãos (ex.: pop-up de detalhe, fora do fluxo de
    // verificarConquistas) — busca direto do documento da pessoa, mesmo
    // padrão já usado abaixo pra dracma_saldo.
    const snap = await getDoc(doc(db, 'users', uid));
    return snap.data()?.streakAtual || 0;
  }
  if (contadorTipo === 'dracma_saldo') {
...
/**
 * Progresso atual de uma conquista, pro contador do pop-up de detalhe
 * (ConquistaDetalheModal) — "quantos a pessoa já fez" sobre "quantos
 * precisa" (conquista.meta), tipo "12/50". Conquistas manuais (sem
 * contadorTipo próprio ou sem meta configurada) não têm essa noção de
 * progresso — retorna null nesse caso, e o pop-up simplesmente não mostra
...
export async function getProgressoConquista(conquista, uid) {
  if (!conquista?.contadorTipo || conquista.contadorTipo === 'manual' || conquista.meta == null) {
    return null;
  }
  try {
...
export async function verificarConquistas(uid, streakAtual, contexto, dracmaSaldoAtual = null) {
  ...
  for (const conquista of candidatas) {
    ...
    if (valorAtual >= conquista.meta) {
      // eslint-disable-next-line no-await-in-loop
      if (await desbloquear(uid, conquista.id)) {
