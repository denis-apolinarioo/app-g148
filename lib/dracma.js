...
export async function creditarDracma(uid, valor, tipo, descricao, referenciaId = null) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    saldoDepois = (snap.data()?.dracmas || 0) + valor;
  ...
  // CONQUISTAS — "Publicano (honesto, viu?)" e demais conquistas ligadas a
  // saldo/ganho de Dracma. Só checa em crédito positivo (evita disparar
  // atoa em eventuais ajustes negativos que passem por aqui).
  if (valor > 0) {
    try {
      await verificarConquistas(uid, 0, 'dracma_ganho', saldoDepois);
    } catch (err) {
      console.error('Erro ao checar conquistas de Dracma:', err);
    }
  }
}
...
/**
 * FASE 3 — remove o Dracma ganho por um post cuja categoria concedia Dracma,
 * quando o post é apagado (dono ou Admin apagando). Espelha
 * removerPontosPost() em lib/points.js — só desconta se o post realmente
 * tinha dracmaGanho gravado (ver lib/acoesLog.js).
 */
export async function removerDracmaPost(uid, postId, dracmaGanho) {
  if (!uid || !dracmaGanho) return;
  await creditarDracma(uid, -dracmaGanho, 'post_removido', 'Post apagado', postId);
}
...
/**
 * Formata um valor de Dracma pra exibição, sempre com 2 casas decimais e
 * vírgula (padrão BR) — ex.: 12 -> "12,00", 7.5 -> "7,50", 3.99 -> "3,99".
 * Usado em qualquer lugar que mostra saldo/valor de Dracma na tela.
 */
export function formatarDracma(valor) {
 ...
 * - 'VALOR_INVALIDO' se o valor não for um número positivo com no máximo
 *   2 casas decimais;
...
export async function transferirDracma(origem, chaveDestino, valor, pin, descricao = '') {
  ...
  // CORREÇÃO: aceita valor quebrado, com no máximo 2 casas decimais (ex.:
  // "12,50", "3.99", "7"). Aceita tanto vírgula quanto ponto como separador
  // decimal (o campo do formulário pode entregar qualquer um dos dois,
  // dependendo do teclado/navegador da pessoa). Arredonda pra 2 casas pra
  // nunca guardar mais precisão do que o combinado.
  const valorNormalizado = String(valor ?? '').trim().replace(',', '.');
  if (!/^\d+(\.\d{1,2})?$/.test(valorNormalizado)) {
    throw new Error('VALOR_INVALIDO');
  }
  const valorNumerico = Math.round(Number(valorNormalizado) * 100) / 100;
  ...
  // CONQUISTAS — do lado de quem ENVIOU (ex.: "dracma_enviado:qtd"): dá pra
  // checar direto aqui, porque quem está rodando esta função é a própria
  // pessoa (mesma sessão, mesmo uid da conquista).
  // BUG CORRIGIDO: essa checagem nunca existia — uma conquista criada pelo
  // Admin com contadorTipo 'dracma_enviado:qtd' nunca desbloqueava pra
  // ninguém, em hipótese nenhuma.
  try {
    await verificarConquistas(origem.uid, 0, 'dracma_enviado');
  } catch (err) {
    console.error('Erro ao verificar conquistas de Dracma enviado:', err);
  }

  // CONQUISTAS — "Publicano" e demais conquistas de Dracma, do lado de quem
  // RECEBEU (inclui "dracma_recebido:qtd"): NÃO dá pra checar/conceder aqui
  // (quem está rodando esta função é quem ENVIOU, e a regra do Firestore só
  // deixa cada pessoa criar a PRÓPRIA conquista — achievementsUnlocked.uid
  // == request.auth.uid). Quem recebeu pega a conquista sozinho, na sessão
  // dele, pelo contexto 'sessao' (ver lib/achievements.js e
...
// ----------------------------------------------------------------------------
// CORREÇÃO DE SEGURANÇA: até esta versão, pinHash/pinSalt (e os campos de
// recuperação) ficavam gravados direto em users/{uid} — coleção com
// `allow read: if isSignedIn()` no firestore.rules (necessário pra Feed,
// Ranking, @menções etc. funcionarem). Isso significava que QUALQUER pessoa
// logada baixava o hash+salt do PIN de QUALQUER outra pessoa (a tela de
// Ranking e a busca de destinatário de transferência já buscam todo mundo
// de `users`), e com o salt em mãos dava pra testar offline as 10.000
// combinações de um PIN de 4 dígitos, sem passar pelo bloqueio de 5
// tentativas — na prática, anulava a proteção.
//
// AGORA: pinHash/pinSalt/tentativas/bloqueio/reset moram numa coleção à
// parte, `walletSecrets/{uid}`, só legível/gravável pelo próprio dono (ver
// firestore.rules). O documento em users/{uid} guarda só um booleano
// `pinConfigurado` (sem valor nenhum de segurança em ficar público — só diz
// "essa pessoa tem PIN", não qual é) pra telas continuarem decidindo
// "criar PIN" vs. "digitar PIN" sem precisar de uma leitura extra.
//
// COMPATIBILIDADE: quem já tinha um PIN configurado antes desta correção
// continua com ele funcionando normalmente — validarPin() abaixo migra os
// dados automaticamente (copia pra walletSecrets, some com o campo antigo)
// na primeira vez que a pessoa usa o PIN dela depois do app atualizado.
// Ninguém precisa recriar o PIN nem perde acesso à Carteira.
// ----------------------------------------------------------------------------
...
function walletSecretsRef(uid) {
  return doc(db, 'walletSecrets', uid);
}

// Campos antigos de PIN que podem ainda existir em users/{uid} (formato
// anterior à correção de segurança) — usado só pra limpar o perfil depois
// que os dados equivalentes já foram gravados em walletSecrets/{uid}.
const CAMPOS_PIN_LEGADO = {
  pinHash: deleteField(),
  pinSalt: deleteField(),
  pinTentativasFalhas: deleteField(),
  pinBloqueadoAte: deleteField(),
  pinResetHash: deleteField(),
  pinResetSalt: deleteField(),
  pinResetExpiraEm: deleteField(),
};
...
/**
 * `perfil` é o objeto de users/{uid} (o mesmo já carregado pra tela — não
 * dispara leitura nova). Cobre tanto quem já está no formato novo
 * (`pinConfigurado: true`) quanto quem ainda não passou pela migração
 * automática (formato antigo, `pinHash` direto no perfil).
 */
export function pinConfigurado(perfil) {
  return !!(perfil?.pinConfigurado || perfil?.pinHash);
}

/**
 * Quantos minutos faltam pro bloqueio por tentativas erradas acabar.
 * Retorna 0 se não está bloqueado. Recebe o documento de walletSecrets
 * (ou, durante a migração, o formato antigo equivalente) — não o perfil.
 */
export function minutosRestantesBloqueio(dadosPin) {
  const ate = dadosPin?.pinBloqueadoAte?.toMillis?.();
  if (!ate) return 0;
 ...
 * próprio usuário. Zera tentativas/bloqueio, já que é um PIN novo. Grava em
 * walletSecrets/{uid} (só o dono lê) e marca `pinConfigurado: true` no
 * perfil; se ainda sobrava algum campo do formato antigo no perfil (pessoa
 * trocando de PIN antes de ter validado o antigo uma vez sequer), limpa
 * junto.
 */
export async function configurarPin(uid, pin) {
  ...
  await setDoc(
    walletSecretsRef(uid),
    { pinHash: hash, pinSalt: salt, pinTentativasFalhas: 0, pinBloqueadoAte: null },
    { merge: true }
  );

  await updateDoc(doc(db, 'users', uid), { pinConfigurado: true, ...CAMPOS_PIN_LEGADO });
}

/**
 * Garante que os segredos do PIN de `uid` estão em walletSecrets/{uid}.
 * Se a pessoa ainda está no formato antigo (pinHash direto em users/{uid},
 * de antes da correção de segurança), copia pra cá e limpa o perfil — só
 * roda essa migração 1 vez por pessoa (a próxima chamada já encontra
 * walletSecrets/{uid} existente e não faz nada). Retorna os dados do PIN
 * (de onde quer que tenham vindo) ou `null` se a pessoa nunca configurou
 * PIN nenhum.
 */
async function garantirWalletSecrets(uid) {
  const secretsRef = walletSecretsRef(uid);
  const snapSecrets = await getDoc(secretsRef);
  const dadosSecrets = snapSecrets.exists() ? snapSecrets.data() : null;
  // Não basta o documento existir: solicitarRecuperacaoPin() pode ter criado
  // ele só com os campos de reset (pinResetHash/Salt/ExpiraEm), sem nunca
  // ter passado por aqui — nesse caso ainda falta migrar o pinHash de
  // verdade, se houver, do formato antigo.
  if (dadosSecrets?.pinHash) return dadosSecrets;

  const userRef = doc(db, 'users', uid);
  const snapUsuario = await getDoc(userRef);
  const dadosUsuario = snapUsuario.data();
  if (!dadosUsuario?.pinHash) return dadosSecrets;

  const migrado = {
    pinHash: dadosUsuario.pinHash,
    pinSalt: dadosUsuario.pinSalt,
    pinTentativasFalhas: dadosUsuario.pinTentativasFalhas || 0,
    pinBloqueadoAte: dadosUsuario.pinBloqueadoAte || null,
  };
  // merge:true — se por acaso já existir um código de recuperação pendente
  // nesse documento (ver comentário acima), preserva em vez de sobrescrever.
  await setDoc(secretsRef, migrado, { merge: true });
  await updateDoc(userRef, { pinConfigurado: true, ...CAMPOS_PIN_LEGADO });
  return { ...dadosSecrets, ...migrado };
}
...
export async function validarPin(uid, pin) {
  const dadosAtuais = await garantirWalletSecrets(uid);
  ...
  return runTransaction(db, async (transaction) => {
    const snap = await transaction.get(secretsRef);
    const dados = snap.data();

    if (hashTentativa === dados.pinHash) {
      transaction.update(secretsRef, { pinTentativasFalhas: 0, pinBloqueadoAte: null });
      return true;
    }
...
export async function confirmarRecuperacaoPin(uid, codigoDigitado) {
  ...
  // O perfil também pode ainda ter os campos do formato antigo (pessoa que
  // nunca chegou a validar o PIN dela pra disparar a migração automática)
  // — limpa os dois lugares, e marca pinConfigurado: false até a pessoa
  // criar o novo PIN em seguida (fluxo já leva direto pra isso).
  await updateDoc(doc(db, 'users', uid), { pinConfigurado: false, ...CAMPOS_PIN_LEGADO });
}
