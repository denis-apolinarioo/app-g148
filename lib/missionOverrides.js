// ============================================================================
// Ajuste de pontos de "outras ações" (fora as missões — hoje só existe
// "postarNoFeed") que o Admin pode alterar depois que o app já está no ar,
...
// "postarNoFeed", que não é uma missão e não tem documento próprio.
// ============================================================================
...
export async function getPontosEfetivos() {
  ...
  try {
    ...
    // Só "postarNoFeed" pode vir de overrides agora — pontos de missão já
    // vêm direto do documento dela (não existe mais override separado).
    return { ...padrao, postarNoFeed: overrides.postarNoFeed ?? padrao.postarNoFeed };
  } catch (err) {
...
/**
 * Usado pelo Painel Admin pra salvar um novo valor de "postarNoFeed". Só
 * deve ser chamado por um usuário admin (a regra de segurança do Firestore
 * também exige isso, como dupla proteção).
 */
