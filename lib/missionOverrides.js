...
export async function getPontosEfetivos() {
  ...
  const padrao = {
    postarNoFeed: PONTOS.postarNoFeed,
    orarPorAlguem: PONTOS.orarPorAlguem,
    abrirVersiculo: PONTOS.abrirVersiculo,
  };
  missoes.forEach((m) => {
  ...
  try {
    ...
    return {
      ...
      abrirVersiculo: overrides.abrirVersiculo ?? padrao.abrirVersiculo,
    };
  } catch (err) {
...
/**
 * Pontos efetivos de "abrirVersiculo" — concedidos na 1ª abertura do
 * pergaminho do versículo do dia (por pessoa, por dia). Mesmo formato de
 * getPontosEfetivosOrarPorAlguem().
 */
export async function getPontosEfetivosAbrirVersiculo() {
  try {
    const snap = await getDoc(doc(db, ...CONFIG_REF_PATH));
    if (snap.exists() && typeof snap.data().abrirVersiculo === 'number') {
      return snap.data().abrirVersiculo;
    }
  } catch (err) {
    console.error('Não foi possível carregar ajuste de pontos, usando padrão:', err);
  }
  return PONTOS.abrirVersiculo;
}

const ROTULOS_ACAO_PONTOS = {
  postarNoFeed: 'Post no Feed',
  orarPorAlguem: 'Orar por alguém',
  abrirVersiculo: 'Abrir o versículo do dia',
};
 ...
 * proteção).
 */
export async function salvarPontosDaMissao(missaoId, novoValor, admin) {
  const snap = await getDoc(doc(db, ...CONFIG_REF_PATH));
  const atual = snap.exists() ? snap.data() : {};
  const valorAntes = atual[missaoId] ?? PONTOS[missaoId] ?? null;
  ...
  if (admin) {
    await registrarAcaoAdmin({
      ...
      detalhes: ROTULOS_ACAO_PONTOS[missaoId] || missaoId,
    });
  }
}
