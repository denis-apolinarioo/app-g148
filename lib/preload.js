...
/**
 * Busca os posts mais recentes do Mural (mesma consulta que subscribeToFeed
 * usa no Feed, só que uma única vez em vez de uma escuta em tempo real) e já
 * deixa em cache: os autores (getUsuarioCache) e a foto de cada post
 * (getCachedImageURL), pra quando o FeedPage montar de verdade tudo já
 * estar pronto e a lista aparecer sem nenhum "piscar" de carregamento.
 * Best effort: nunca lança erro pra fora — se o Firebase falhar aqui, o
 * FeedPage simplesmente busca do zero quando abrir, normalmente.
 */
export async function preloadFeedInicial(quantidade = 15) {
  try {
    ...
    console.warn('Não foi possível pré-carregar o feed inicial:', err);
  }
}
