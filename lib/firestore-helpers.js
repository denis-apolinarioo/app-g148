...
// Mesmo motivo: sem orderBy no Firestore, ordena no cliente.
export function subscribeToUserChallenges(uid, callback) {
  const q = query(collection(db, 'challenges'), where('destinatarioId', '==', uid));
  return onSnapshot(
    q,
    (snap) => {
      reportarConexaoOk();
    ...
    (err) => {
      console.error('[subscribeToUserChallenges] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
...
export function subscribeToPendingChallenges(callback) {
  ...
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeToPendingChallenges] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
...
export async function marcarDenunciaResolvida(reportId, resolvida = true) {
