...
export async function createPost({
  ...
  const postRef = await addDoc(collection(db, 'posts'), {
    ...
    itens: itens || null,
    // CORREÇÃO DE BUG (botão "encaminhar" quebrado em missão que não
    // posta) — post "de bastidor" gerado por uma missão com
    // postaNoFeed:false (ver criarPostDeMissao em lib/points.js). `true`
    // faz Feed/Busca/Perfil ignorarem ele (filtro no cliente, mesmo padrão
    // já usado pra `oculto` — ver app/(app)/feed/page.js,
    // app/(app)/buscar/page.js e components/ProfileView.js) e o PostCard
    // esconder curtida/comentário (ver components/PostCard.js). `false`
    // (padrão) em post manual e em post de missão que publica normalmente.
    missaoSemFeed: !!missaoSemFeed,
    curtidas: [],
    comentariosCount: 0,
    createdAt: serverTimestamp(),
  });
 ...
 * Guarda `pontosGanhos`/`pointsLogId` e (se o Admin tiver habilitado
 * Dracma pra essa ação) `dracmaGanho`/`dracmaLogId` dentro do próprio
 * documento da interação — é o que permite a desfazerOracao() (o
 * "desclique") reverter depois exatamente o que foi ganho, nem mais nem
 * menos.
 ...
 * que estavam salvos ({pontosGanhos, pointsLogId, dracmaGanho,
 * dracmaLogId}) pra quem chamou poder reverter os pontos/Dracma também
 * (lib/points.js), ou `null` se não havia oração hoje pra desfazer (ex.:
 * clique duplo).
 */
export async function desfazerOracao(prayerId, uid) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    return {
      ...
      pointsLogId: dados.pointsLogId || null,
      dracmaGanho: dados.dracmaGanho || 0,
      dracmaLogId: dados.dracmaLogId || null,
    };
  });
}
...
/**
 * Versão "ao vivo" de getAllUsers — escuta a coleção "users" via onSnapshot
 * em vez de buscar uma vez, pra lista principal do painel Admin (aba
 * Usuários) refletir mudanças na hora. Usada só onde a lista fica na tela
 * sendo observada — buscas/seletores pontuais (escolher destinatário de
 * missão, correio, etc.) continuam com getAllUsers() de sempre, sem
 * necessidade de ficar "ao vivo". Retorna a função de cancelar a escuta.
 */
export function subscribeToAllUsers(callback) {
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeToAllUsers] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
