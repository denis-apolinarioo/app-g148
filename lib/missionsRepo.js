...
function valoresPadrao() {
  return {
    ...
    // Recompensa: mesmo padrão de lib/categoriasAcaoRepo.js — pode pontuar,
    // dar Dracma, os dois, ou nenhum dos dois, cada um independente.
    // `pontua` não existia antes desta opção; ausente conta como true (ver
    // leituras com `!== false`) pra missão antiga continuar pontuando igual.
    pontua: true,
    daDracma: false,
    dracma: 0,
    repeteAutomaticamente: true,
    vezesPorPeriodo: 1,
    // Horário de visibilidade: se horarioAtivo for false (padrão), a missão
    // fica visível o dia inteiro. Se true, só aparece pro usuário entre
    // horarioInicio e horarioFim ("HH:MM", fuso de Brasília) — ver
    // lib/dateUtils.js (dentroDaJanelaHorario).
    horarioAtivo: false,
    horarioInicio: null,
    horarioFim: null,
  };
...
/**
 * Versão "ao vivo" de getTodasAsMissoes — em vez de buscar uma vez, escuta a
 * coleção inteira via onSnapshot e chama `callback` com a lista atualizada
 * (já ordenada por `ordem`) toda vez que algo mudar no Firestore, não
 * importa quem mudou (o próprio Admin arrastando, outro Admin em outra aba,
 * uma missão sendo criada/editada/apagada). Retorna a função de cancelar a
 * escuta — quem chama PRECISA desinscrever no cleanup do useEffect, senão a
 * escuta continua rodando depois do componente sair de tela. Mesmo padrão
 * de subscribeToUserProfile (lib/firestore-helpers.js) e da página de
 * Ranking.
 */
export function subscribeATodasAsMissoes(callback) {
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeATodasAsMissoes] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
...
/**
 * CORREÇÃO DE BUG: apagar uma missão só apagava o documento dela na coleção
 * "missoes" — o histórico de missionSubmissions (quem cumpriu, quando,
 * com uid + missaoId apontando pra essa missão) ficava pra trás, órfão.
 * Dava pra ver isso, por exemplo, na busca "por pessoa" do Admin: a missão
 * some da lista de escolher, mas a pessoa continuava aparecendo com um
 * registro cujo "título" caía no fallback (o próprio ID), porque a missão
 * que ele deveria buscar já não existia mais.
 *
 * Agora apaga também TODAS as submissões dessa missão (de qualquer usuário,
 * de qualquer ciclo), em lotes de 450 (o limite do Firestore é 500
 * operações por lote — 450 dá uma margem de segurança). Isso NÃO desfaz
 * pontos/Dracma que as pessoas já ganharam (o saldo de cada um continua
 * como está, mesma filosofia de apagarCategoriaAcao em
 * lib/categoriasAcaoRepo.js) nem apaga os posts que a missão gerou — só o
 * registro de rastreio/histórico que apontava pra uma missão que não vai
 * mais existir.
 */
export async function apagarMissao(missaoId, admin, tituloMissao) {
  const snapSubmissoes = await getDocs(
    query(collection(db, 'missionSubmissions'), where('missaoId', '==', missaoId))
  );
  const refsSubmissoes = snapSubmissoes.docs.map((d) => d.ref);
  for (let i = 0; i < refsSubmissoes.length; i += 450) {
    const lote = refsSubmissoes.slice(i, i + 450);
