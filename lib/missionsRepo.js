 ...
 * `campos`. Roda a partir de um botão no painel Admin, no mesmo espírito de
 * migrarMissoesDoCodigoParaFirestore(). Segura pra clicar mais de uma vez:
 * só mexe em documentos que ainda tenham o campo antigo `tipo` — quem já
 * foi migrado (tem `campos` e não tem mais `tipo`) é ignorado.
 */
export async function migrarCamposDasMissoes() {
  ...
  for (const missao of todas) {
    ...
    if (missao.tipo === 'check') {
      campos = [
        { chave: 'confirmacao', label: missao.perguntaConfirmacao || '', tipo: 'check' },
      ];
    } else if (missao.tipo === 'texto' || missao.tipo === 'reflexao') {
      campos = missao.campos || [];
    } else if (missao.tipo === 'leitura') {
      campos = [
        { chave: 'concluido', label: 'Confirmo que terminei a leitura', tipo: 'check' },
      ];
