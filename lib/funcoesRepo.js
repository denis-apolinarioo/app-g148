...
/**
 * Versão "ao vivo" de getTodasAsFuncoes — escuta a coleção via onSnapshot em
 * vez de buscar uma vez, pro painel Admin (aba Funções) e o seletor de
 * função (editar perfil) refletirem mudanças na hora, sem sair-e-voltar.
 * `garantirSeed()` roda uma vez, "e esquece" (idempotente — só grava se a
 * coleção ainda estiver vazia); assim que terminar, a própria escuta já
 * ativa pega o resultado sozinha. Retorna a função de cancelar a escuta.
 * Mesmo padrão de lib/missionsRepo.js (subscribeATodasAsMissoes).
 */
export function subscribeATodasAsFuncoes(callback) {
  garantirSeed().catch((err) => console.error('[subscribeATodasAsFuncoes] Erro ao garantir seed:', err));
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeATodasAsFuncoes] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
...
/**
 * Item novo — grava a `ordem` de TODAS as funções de uma vez, na ordem já
 * definida pela pessoa que arrastou (drag-and-drop no Admin). Mesmo padrão
 * de lib/categoriasAcaoRepo.js (reordenarCategoriasAcao) — trocarOrdemFuncao
 * acima continua servindo só pras setinhas (troca 2 de cada vez).
 */
export async function reordenarFuncoes(funcoesNaNovaOrdem) {
  const batch = writeBatch(db);
  funcoesNaNovaOrdem.forEach((funcao, index) => {
    batch.update(doc(db, COLECAO, funcao.id), { ordem: index });
  });
  await batch.commit();
}
