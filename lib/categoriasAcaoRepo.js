...
function valoresPadrao() {
  return {
    ...
    exigeTexto: false,
    exigeAudio: false,
    // Horário de visibilidade: se horarioAtivo for false (padrão), a
    // categoria fica disponível o dia inteiro. Se true, só aparece no
    // seletor de categoria entre horarioInicio e horarioFim ("HH:MM", fuso
    // de Brasília) — ver lib/dateUtils.js (dentroDaJanelaHorario).
    horarioAtivo: false,
    horarioInicio: null,
    horarioFim: null,
  };
...
/**
 * Versão "ao vivo" de getTodasAsCategoriasAcao — escuta a coleção via
 * onSnapshot em vez de buscar uma vez, pro painel Admin (aba Ações) refletir
 ...
 */
export function subscribeATodasAsCategoriasAcao(callback) {
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeATodasAsCategoriasAcao] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
