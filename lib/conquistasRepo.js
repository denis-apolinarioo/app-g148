...
//   emblema     — tier visual opcional: 'ferro' | 'bronze' | 'prata' | 'ouro'
//                 | 'diamante' | '' (nenhum). Ver lib/emblemas.js — é a
//                 moldura/"capa 3d" que entra por trás da imagemURL/ícone,
//                 com animação de brilho quando a conquista tá desbloqueada.
//   contadorTipo — CLÁSSICA (um contador só): 'streak' | 'oracao' | 'post' |
//                 'missao:<id>' | 'dracma_saldo' | 'dracma_enviado:qtd' |
//                 'dracma_recebido:qtd' | 'dracma_ganho_total' | 'manual'
//                 (lista completa em lib/achievements.js -> calcularContador)
//   meta        — CLÁSSICA: quantidade necessária (null quando contadorTipo
//                 é 'manual'). Com `contadores` preenchido (ver abaixo),
//                 vira "quantas contagens completas do combo são
//                 necessárias" (default 1 se ausente).
//   ativa       — só conquistas ativas contam no motor de verificação
//   ordem       — ordem de exibição no catálogo
//   criadaEm    — gravado automaticamente por criarConquista() (serverTimestamp)
//
//   ATUALIZAÇÃO CONQUISTAS — campos novos, todos OPCIONAIS (ausentes =
//   comportamento clássico de sempre, sem migração nenhuma nas 65
//   conquistas já existentes):
//   contadores  — array de { tipo, meta, modo }, pra conquista de MÚLTIPLOS
//                 contadores (só conta quando TODOS baterem a própria
//                 meta). Quando preenchido, SUBSTITUI contadorTipo/meta —
//                 ver avaliarConquistaMultiContador em lib/achievements.js.
//                 `tipo` usa o mesmo catálogo de contadorTipo. `modo` é
//                 'estado_atual' | 'obtido_periodo' | 'lancamento'.
//   periodoValidacaoDias — nº de dias (>0) que a pessoa tem pra cumprir
//                 TODOS os `contadores` a partir do 1º progresso dela nessa
//                 conquista (prazo PESSOAL, não é um relógio único pra todo
//                 mundo — ver comentário grande em
//                 avaliarConquistaMultiContador). Ausente/0 = sem prazo
//                 (só exige que todos os contadores estejam cumpridos AGORA,
//                 sem repetição). Só faz sentido junto de `contadores`.
//   pontosRecompensa, dracmaRecompensa — anexados à mensagem do Correio que
//                 avisa o desbloqueio (automático OU manual — ver
//                 notificarConquista()/desbloquear() em lib/achievements.js),
//                 e só creditados quando a pessoa toca "Receber" naquela
//                 mensagem (resgatarRecompensaCorreio, lib/notificacoes.js).
//                 Ausente/0 = sem recompensa (comportamento de sempre —
//                 desbloquear conquista era 100% cosmético antes desta
//                 atualização).
// ============================================================================
import {
  collection,
...
/**
 * Versão "ao vivo" de getTodasAsConquistas — escuta a coleção via onSnapshot
 * em vez de buscar uma vez, pro painel Admin (aba Conquistas) refletir
 * mudanças na hora, sem sair-e-voltar. Retorna a função de cancelar a
 * escuta. Mesmo padrão de lib/missionsRepo.js (subscribeATodasAsMissoes).
 */
export function subscribeATodasAsConquistas(callback) {
  return onSnapshot(
    ...
    (err) => {
      console.error('[subscribeATodasAsConquistas] Erro na escuta em tempo real:', err);
      reportarErroConexao();
    }
  );
}
