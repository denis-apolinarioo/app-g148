 ...
 * de chegar aqui.
 */
async function criarPostDeMissao(missao, autor, resposta, fotoURL, audioURL, submissaoId) {
  ...
  // usado por post com categoria (ver CreatePostSheet.js).
  if (postId && missao.pontos) {
    await updateDoc(doc(db, 'posts', postId), { pontosGanhos: missao.pontos });
  }
...
export async function submeterMissao(missaoId, autor, resposta, fotoURL, audioURL, audioDuracaoSegundos) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    transaction.set(submissionRef, {
      ...
      // Preenchido logo abaixo, depois que o post automático é criado (se a
      // missão publica no Feed) — permite "encaminhar" desta submissão pro
      // post que ela gerou (ver getSubmissoesDoCicloComPost em
      // lib/missionCycles.js e o botão de atalho em MissionCard.js).
      postId: null,
      createdAt: serverTimestamp(),
    });
  });
  ...
  if (missao.postaNoFeed) {
    const postId = await criarPostDeMissao(missao, autor, resposta, fotoURL, audioURL, submissionRef.id);
    if (postId) {
 ...
 *
 * `pontosGanhos` vem do próprio post (post.pontosGanhos) — se por acaso
 * vier vazio (post antigo, de antes desta correção), cai pro valor atual
 * configurado na missão, pra não deixar de descontar nada.
 */
export async function removerPontosMissaoDoPostApagado(uid, missaoId, submissaoId, pontosGanhos) {
  if (!uid || !missaoId || !submissaoId) return;

  const missao = await getMissaoPorId(missaoId);
  const valorADescontar = pontosGanhos || missao?.pontos || 0;

  if (valorADescontar) {
    await awardPoints(uid, -valorADescontar, 'missao_removida', 'Missão apagada', submissaoId);
  }
...
export async function pontuarOracao(uid, prayerId) {
  ...
  const registrou = await registrarOracao(prayerId, uid, pontosEfetivos, logRef.id);
  if (!registrou) return false;

  const userRef = doc(db, 'users', uid);
  await runTransaction(db, async (transaction) => {
    transaction.update(userRef, { pontos: increment(pontosEfetivos) });
    transaction.set(logRef, {
      uid,
      tipo: 'oracao',
...
export async function desfazerPontosOracao(uid, prayerId) {
  ...
  const { pontosGanhos, pointsLogId } = desfeito;
  const userRef = doc(db, 'users', uid);
  await runTransaction(db, async (transaction) => {
    if (pontosGanhos) {
...
/**
 * Pontos por abrir o pergaminho do versículo do dia (components/
 * VersiculoDiario.js). Só concede na PRIMEIRA abertura de cada dia (fuso
 * Brasília) — abrir/fechar várias vezes no mesmo dia não pontua de novo.
 * O controle é o campo `ultimoVersiculoAbertoEm` (formato 'YYYY-MM-DD') em
 * users/{uid}, checado e gravado na MESMA transação que credita o ponto,
 * pra não haver corrida entre checar e gravar.
 *
 * Retorna quantos pontos foram concedidos (0 se já tinha aberto hoje).
 */
export async function pontuarAberturaVersiculo(uid) {
  const hoje = todayBrasilia();
  const userRef = doc(db, 'users', uid);
  const pontosEfetivos = await getPontosEfetivosAbrirVersiculo();
  const logRef = doc(collection(db, 'pointsLog'));
  let creditou = false;
  ...
  await runTransaction(db, async (transaction) => {
    const snap = await transaction.get(userRef);
    if (!snap.exists()) return;
    if (snap.data()?.ultimoVersiculoAbertoEm === hoje) return; // já pontuou hoje
    ...
    transaction.set(logRef, {
      ...
      tipo: 'versiculo',
      valor: pontosEfetivos,
      descricao: 'Abriu o versículo do dia',
      referenciaId: null,
      createdAt: serverTimestamp(),
    });
    creditou = true;
  ...
  return creditou ? pontosEfetivos : 0;
}
