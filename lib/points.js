...
import { getPontosEfetivosDeUmaMissao, getPontosEfetivosPostarNoFeed } from './missionOverrides';
...
export async function getStatusMissoesDiariasHoje(uid) {
  ...
  const resultados = {};
  await Promise.all(
    MISSOES_DIARIAS.map(async (missao) => {
 ...
 * `fotoURL` já deve vir pronta (upload feito antes, ver lib/imageCompress.js
 * + upload no Storage).
 *
 * Lança erro 'MISSAO_JA_ENVIADA_HOJE' se a pessoa já enviou essa missão hoje.
 */
export async function submeterMissaoDiaria(missaoId, autor, resposta, fotoURL) {
  const missao = MISSOES_DIARIAS.find((m) => m.id === missaoId);
  ...
  const pontosEfetivos = await getPontosEfetivosDeUmaMissao(missaoId, missao.pontos);
  ...
  try {
    ...
    if (err.message === 'MISSAO_JA_ENVIADA_HOJE') throw err;
    throw err;
  }

  await awardPoints(autor.uid, pontosEfetivos, 'missao_diaria', missao.titulo, submissionId);
  await atualizarStreak(autor.uid);

  if (missao.postaNoFeed) {
    const texto = Object.values(resposta || {}).filter(Boolean).join('\n\n');
    await createPost({
      autor,
      tipo: fotoURL ? 'foto' : 'texto',
      texto: `${missao.titulo}\n\n${texto}`,
      midiaURL: fotoURL || '',
 ...
 */
export async function submeterMissaoSemanal(missaoId, autor, resposta, fotoURL) {
  const missao = MISSOES_SEMANAIS.find((m) => m.id === missaoId);
  if (!missao) throw new Error('MISSAO_INVALIDA');
  ...
  await awardPoints(autor.uid, pontosEfetivos, 'missao_semanal', missao.titulo, submissionId);
  await atualizarStreak(autor.uid);
 ...
 */
export async function concluirMissaoMensal(missaoId, autor) {
  const missao = MISSOES_MENSAIS.find((m) => m.id === missaoId);
  if (!missao) throw new Error('MISSAO_INVALIDA');
  ...
  await awardPoints(autor.uid, pontosEfetivos, 'missao_mensal', missao.titulo, submissionId);
  return true;
}

/**
 * Pontos por postar espontaneamente no feed (fora do fluxo de missões).
 * ATUALIZAÇÃO: o valor agora é configurável pelo Admin (getPontosEfetivosPostarNoFeed)
 * em vez de fixo em PONTOS.postarNoFeed. O valor efetivamente usado fica
 * gravado no próprio post (pontosGanhos), pra que, se o post for apagado
 * depois, dê pra remover exatamente esses pontos — mesmo que o Admin mude
 * a pontuação padrão nesse meio tempo.
 */
...
export async function pontuarOracao(uid, prayerId) {
  await awardPoints(uid, PONTOS.orarPorAlguem, 'oracao', 'Orou por um pedido', prayerId);
}
...
export async function aprovarDesafio(challengeId, uid, pontos) {
  await updateDoc(doc(db, 'challenges', challengeId), { status: 'aprovado' });
  await awardPoints(uid, pontos, 'desafio', 'Desafio individual aprovado', challengeId);
}
