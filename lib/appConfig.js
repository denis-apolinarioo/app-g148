...
// PACOTE 3, item 3.1 — liga/desliga a função de transferir Dracma entre
// usuários (Carteira → Enviar Dracma). Mesmo padrão do toggle acima: some
// do documento até o Admin mexer pela 1ª vez, então o padrão (chave
// ausente) é `true` — a função nasce ligada.
export const CHAVE_TRANSFERENCIA_DRACMA_ATIVA = 'transferenciaDracmaAtiva';

// Aba Estética — id do preset de cor usado como padrão do app (quem ainda
// não escolheu um preset manualmente no próprio aparelho vê esse). Some do
// documento até o Admin usar "Definir como padrão" pela 1ª vez, por isso o
// padrão (chave ausente) é 'claro' — mantém o comportamento de sempre.
export const CHAVE_PRESET_PADRAO = 'presetPadraoId';

// Item novo — nome da comunidade, mostrado no cabeçalho do Feed, na tela de
// login e (quando ele voltar a existir) na splash. Some do documento até o
// Admin preencher pela 1ª vez, por isso o padrão (chave ausente) é
// NOME_APP_PADRAO — pensando já na frente pro multi-tenant (cada comunidade
// vai ter o próprio nome; "Community" é um nome genérico de sobra até lá,
// não precisa ficar hardcoded "Geração 148" pra sempre em cada instalação
// nova). NÃO cobre o título da aba do navegador nem o nome do PWA ao
// "adicionar à tela inicial" — esses dois vêm de app/layout.js (metadata)
// e metadata.json, arquivos fixos do projeto, exigem novo deploy pra mudar
// (não são dado de comunidade, são configuração de código).
export const CHAVE_NOME_COMUNIDADE = 'nomeComunidade';
export const NOME_APP_PADRAO = 'Community';
