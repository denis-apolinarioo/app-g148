...
/**
 * Quantos px a moldura do tier "vaza" abaixo do círculo, pra um dado
 * tamanho de selo — usada por quem posiciona algo logo abaixo do emblema
 * (nome no grid, na vitrine etc.) pra não colar no anel, já que a moldura
 * é sempre maior que o próprio círculo e desliza pra fora dele (ver
 * comentário grande no topo deste arquivo). Sem tier (`tierId` nulo/
 * inexistente), retorna 0 — círculo simples não vaza.
 */
export function overflowMoldura(tierId, size) {
  const tier = tierId ? EMBLEMAS_POR_ID[tierId] : null;
  if (!tier) return 0;
  const escala = parseFloat(tier.moldura.escala) / 100;
  const deslocY = parseFloat(tier.moldura.deslocY) / 100;
  return Math.max(0, Math.round(size * (escala + deslocY - 1)));
}
