...
/**
 * Capa de um Material (painel Admin > Materiais, ícone de caixa no
 * Perfil). Corte fixo 1:1 pelo ImageCropper, igual à imagem circular de
 * conquista — mesmo padrão de nome fixo por material (trocar a capa
 * substitui a anterior em vez de acumular arquivo antigo). `materialId` é
 * o ID do documento (gerado antes de subir a capa, ver lib/materiaisRepo.js).
 */
export async function uploadCapaMaterial(materialId, arquivo) {
  // jaVerificado: true — vem do ImageCropper (corte 1:1 fixo em
  // AbaMateriais.js). Ver raciocínio completo em
  // uploadFotoCorreioComThumb, mais abaixo.
  const comprimida = await comprimirImagem(arquivo, { jaVerificado: true });
  const storageRef = ref(storage, `materiais/${materialId}/capa.jpg`);
  await uploadBytes(storageRef, comprimida, {
    contentType: comprimida.type || 'image/jpeg',
    cacheControl: 'public, max-age=86400',
  });
...
/**
 * Conteúdo de um Material do tipo "imagem" — a imagem que abre em tela
 * cheia quando a pessoa toca no material (ver ImageViewerModal). NÃO
 * passa pelo ImageCropper (sem corte fixo) de propósito: pode ser um
 * cartaz/infográfico em qualquer proporção — diferente da capa, que é
 * sempre 1:1. Mesmo nome fixo por material que a capa (trocar substitui
 * o arquivo anterior).
 */
export async function uploadConteudoImagemMaterial(materialId, arquivo) {
  const comprimida = await comprimirImagem(arquivo);
  const storageRef = ref(storage, `materiais/${materialId}/conteudo.jpg`);
  await uploadBytes(storageRef, comprimida, {
    contentType: comprimida.type || 'image/jpeg',
    cacheControl: 'public, max-age=86400',
  });
...
/**
 * Conteúdo de um Material do tipo "pdf" — enviado como veio, sem
 * compressão (PDF não é imagem, não passa por comprimirImagem). Abre numa
 * aba nova (ver app/(app)/materiais/page.js) — o navegador do celular ou
 * do computador já tem leitor de PDF embutido, então não precisa de
 * nenhuma biblioteca nova no app pra isso funcionar.
 */
export async function uploadConteudoPdfMaterial(materialId, arquivo) {
  const storageRef = ref(storage, `materiais/${materialId}/conteudo.pdf`);
