...
// ----------------------------------------------------------------------------
// Pede ao navegador pra tratar o armazenamento deste app como "persistente"
// — sem isso, o sistema operacional pode limpar o Cache Storage sozinho,
// sem avisar ninguém, se o aparelho ficar com pouco espaço (é o que evita
// as fotos "sumirem" do cache depois de alguns dias de uso).
// Chamar 1x só, no carregamento do app (ver AuthProvider.js). Não tem efeito
// nenhum se o navegador não suportar a API — nesse caso só ignora, sem quebrar.
// Observação: no iPhone/iPad (Safari), essa chamada não existe — lá, a
// garantia real é o app estar instalado na tela de início (modo standalone),
// que sozinho já livra o site do limite de 7 dias sem uso do Safari normal.
// ----------------------------------------------------------------------------
let jaSolicitouPersistencia = false;
export async function solicitarArmazenamentoPersistente() {
  if (jaSolicitouPersistencia) return;
  jaSolicitouPersistencia = true;
  try {
    if (navigator?.storage?.persist) {
      await navigator.storage.persist();
    }
  } catch {
    // best effort — se o navegador negar ou não suportar, o app segue
    // funcionando normal, só sem essa garantia extra
  }
}
