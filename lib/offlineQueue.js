...
import { toggleLike, toggleCommentLike, registerPrayerInteraction } from './firestore-helpers';
...
async function executarAcao({ tipo, payload }) {
  if (tipo === 'curtidaPost') {
    ...
    const registrou = await registerPrayerInteraction(payload.prayerId, payload.uid);
    if (registrou) {
      await pontuarOracao(payload.uid, payload.prayerId);
