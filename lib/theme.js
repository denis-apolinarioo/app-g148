...
function aplicarTema(tema) {
  ...
  // base no <meta name="theme-color">, numa camada separada do conteúdo
  // da página. Ao reabrir o app, um script no <head> já ajusta essa tag
  // ANTES da 1ª pintura, então tudo nasce sincronizado. Mas ao trocar o
  // tema com o app já aberto, só mudar o atributo `content` de uma tag
  // que já existe (setAttribute) nem sempre faz o Chrome no Android
  // repintar a barra na hora — em alguns aparelhos ela demora um instante
  // pra acompanhar, e nesse intervalo aparece uma linha fina onde a cor
  // antiga (barra) encontra a cor nova (conteúdo). Recriar a tag do zero
  // (remover e inserir de novo, em vez de só editar o atributo) é o jeito
  // que faz o navegador perceber a mudança e repintar junto, sem esse
  // atraso.
  const corAntiga = document.querySelector('meta[name="theme-color"]');
  if (corAntiga) corAntiga.remove();
  const novaCor = document.createElement('meta');
  novaCor.setAttribute('name', 'theme-color');
  novaCor.setAttribute('content', tema === 'escuro' ? '#0D0906' : '#FAF6EF');
  document.head.appendChild(novaCor);
}
