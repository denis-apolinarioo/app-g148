...
export function useTema() {
  ...
  const alternar = useCallback(() => {
    ...
    // MÉTODO NOVO (troca do fade antigo, que animava a cor de CADA elemento
    // da tela ao mesmo tempo — pesado, travava no Android por ter que
    // recalcular estilo de centenas de nós a cada frame). A View
    // Transitions API tira um "print" da tela antes e depois da troca, e
    // faz um cross-fade (opacidade) entre as 2 imagens — só isso, roda
    // direto na GPU, sem recalcular nada do layout, então fica suave sem
    // travar. Duração/curva customizadas em globals.css.
    if (typeof document !== 'undefined' && document.startViewTransition) {
      document.startViewTransition(trocar);
    } else {
      // Navegador sem suporte à API (raro): troca na hora, sem animação —
      // melhor que arriscar o fade antigo travando de novo.
      trocar();
    }
  }, []);
