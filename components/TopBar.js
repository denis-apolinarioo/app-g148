...
export default function TopBar({ titulo, voltarPara, voltarPorHistorico, acao }) {
  ...
  return (
    // .barra-fade-tema: dá um fade próprio pra essa barra ao trocar tema
    // (backdrop-blur não entra no cross-fade automático da View Transition
    // — ver comentário da classe em globals.css), sincronizado com o resto
    // da tela pela mesma variável de duração/curva.
    <header className="barra-fade-tema sticky top-0 z-30 flex items-center gap-2 bg-cream/95 px-4 py-3.5 backdrop-blur pt-[calc(env(safe-area-inset-top)+0.875rem)]">
      {voltarPorHistorico ? (
        <button
          type="button"
          onClick={() => router.back()}
