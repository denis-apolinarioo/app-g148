...
const CATEGORIAS = [null, 'Relato', 'Oracao'];
const PROPORCOES = [
  { label: '1:1', w: 1, h: 1 },
  { label: '4:5', w: 4, h: 5 },
  { label: '3:4', w: 3, h: 4 },
...
export default function CreatePostSheet({ onFechar, onPublicado }) {
  ...
  async function handlePublicar() {
    ...
    try {
      ...
      setErro(
        'Nao foi possivel publicar agora. Verifique sua internet.'
      );
    } finally {
      setPublicando(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-coffee-900/40 sm:items-center">
      <div className="max-h-[88vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-cream sm:rounded-2xl">
        <div className="flex items-center justify-between border-b border-coffee-100 px-5 py-4">
          <h2 className="font-destaque text-lg font-semibold text-coffee-800">Nova publicacao</h2>
        ...
        <div className="px-5 py-4">
          <div className="mb-4 flex items-center gap-2 border-b border-coffee-100">
            ...
            <AbaBtn ativo={aba === 'audio'} onClick={() => setAba('audio')} icone={MicIcon} label="Audio" />
          </div>

          <textarea
            value={texto}
            onChange={(e) => setTexto(e.target.value)}
            placeholder={aba === 'texto' ? 'No que voce esta pensando?' : 'Adicione uma legenda (opcional)'}
            rows={4}
          ...
          {aba === 'foto' && (
            <div className="mt-3">
              {previewFoto ? (
                ...
                >
                  <img src={previewFoto} alt="Previa" className="max-h-64 w-full object-cover" />
