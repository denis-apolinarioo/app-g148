...
export default function PostCard({ post, usuarioAtual }) {
  ...
  function renderItemMissao(item, key, comDuploToque) {
    ...
    if (item.tipo === 'audio') {
      return (
        <div key={key} className="relative mb-2.5 rounded-xl border border-coffee-100 bg-cream px-3 py-2" onClick={comDuploToque ? handleDuploToque : undefined}>
  ...
  return (
    <div className="rounded-2xl border border-coffee-100 bg-cream-card p-4">
      ...
      {!itensMissao && post.tipo === 'audio' && post.midiaURL && (
        <div className="relative mb-3 rounded-xl border border-coffee-100 bg-cream px-3 py-2" onClick={handleDuploToque}>
