...
export default function EmblemaConquista({
  ...
  // CORREÇÃO (cache de imagens) — conquista.imagemURL é uma foto do Firebase
  // Storage (subida em lib/storage.js -> uploadImagemConquista), mas até
  // aqui era exibida com <img src={conquista.imagemURL}> direto, sem passar
  // pelo cache persistente em disco (lib/imageCache.js) — diferente do
  // padrão já usado em Avatar.js. Como este componente é a fonte única do
  // selo de conquista (grid do perfil com até dezenas de badges, vitrine,
  // modal de detalhe, seletor de vitrine), a mesma imagem é buscada de novo
  // repetidas vezes sem essa mudança. Mesmo padrão do Avatar.js: mostra a
  // URL original na hora (nunca fica em branco esperando) e troca pela
  // versão em cache assim que ela ficar pronta, sem nenhum "piscar".
  const [imagemCacheada, setImagemCacheada] = useState('');
  useEffect(() => {
    // Só passa pelo cache imagens remotas de verdade (Firebase Storage).
    // Alguns lugares (ex.: preview ao trocar a imagem no formulário de
    // conquista, no Admin) passam uma URL local (blob:) que ainda nem foi
    // salva — cachear isso à toa só gastaria memória sem ganho nenhum
    // (mesmo cuidado já tomado em ImageViewerModal.js).
    if (!conquista.imagemURL || conquista.imagemURL.startsWith('blob:') || conquista.imagemURL.startsWith('data:')) {
      setImagemCacheada('');
      return undefined;
    }
    let cancelado = false;
    getCachedImageURL(conquista.imagemURL).then((url) => {
      if (!cancelado) setImagemCacheada(url);
    });
    return () => {
      cancelado = true;
    };
  }, [conquista.imagemURL]);
  const srcImagemConquista = imagemCacheada || conquista.imagemURL;
        ...
        {precisaCadeado && (
          <div
            className={`absolute inset-0 flex items-center justify-center rounded-full ${
              travada ? 'bg-coffee-50/60' : 'bg-forte-900/30'
