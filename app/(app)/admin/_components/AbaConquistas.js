...
import { getTodasAsMissoes, subscribeATodasAsMissoes } from '@/lib/missionsRepo';
import { getTodasAsCategoriasAcao, subscribeATodasAsCategoriasAcao } from '@/lib/categoriasAcaoRepo';
...
export default function AbaConquistas() {
  ...
  // Escuta ao vivo (onSnapshot) em vez de buscar uma vez só — conquistas,
  // missões e categorias de ação (essas duas últimas só como referência pro
  // formulário) refletem mudanças na hora, sem sair-e-voltar. `carregar`
  // acima continua existindo e sendo chamada em alguns pontos abaixo — com
  // a escuta já ativa isso virou redundante, mas inofensivo.
  useEffect(() => {
    const unsubConquistas = subscribeATodasAsConquistas(setConquistas);
    const unsubMissoes = subscribeATodasAsMissoes(setMissoes);
    const unsubCategorias = subscribeATodasAsCategoriasAcao(setCategoriasAcao);
    return () => {
      unsubConquistas();
      unsubMissoes();
      unsubCategorias();
